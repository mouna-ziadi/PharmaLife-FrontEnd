...
/*  getArticleById(idArticle: number) {
 ...
 getArticleById(idArticle: number): Observable<Article>{
   return this.httpClient.get<Article>(`${this.API_URL}/article/${idArticle}`,this.options);
 }

 addCommentToArticle(comment: Commentaire, idArticle:number) : Observable<any>{
  return this.httpClient.post(`${this.API_URL}/articles/${idArticle}/comments`, comment,this.options);
